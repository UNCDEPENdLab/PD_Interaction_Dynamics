...
# dataFile <- df
# (length(df[[1]]))
# dyn_models <- function(dataFile, subSet)  {
#   numVars <- length(subSet)
#   df <- read.table(file.path(dirPath, dataFile), col.names(c(strsplit(subSet[[1]]:subSet[[numVals]]), "\\s+") [[1L]]))
#   #df$time <- seq(1,nrow(df)/1, by =1 )
#   names(df) <- gsub("_", ".", names(df), fixed = TRUE)
#   df$id <- dataFile$PTNUM[[1]]
...
#   df[,c("l.ibi.interp.detrend", "r.ibi.interp.detrend","l.dom.interp.detrend", "r.dom.interp.detrend", "l.aff.interp.detrend", "r.aff.interp.detrend")] <-
#     lapply(df[,c("l.ibi.interp.detrend", "r.ibi.interp.detrend","l.dom.interp.detrend", "r.dom.interp.detrend", "l.aff.interp.detrend", "r.aff.interp.detrend")], scale)
#   time2 = rep(1:length(subSet[[1]]),each=10)
#   
...
#   
#   nameVars <- for (i in 1:numVars) {
#     
#   }
#   
#   data <- dynr.data(df, id="id", time="time",
#                     observed=c(ts1, ts2))
#   cor(df[,c(subSet[[1]]:subSet[[numVals]])])
#   meas <- prep.measurement(
#     values.load=diag(1,numVars), # starting values and fixed values
#     params.load=diag(rep("fixed",numVars),numVars),
#     state.names=c(subSet[[1]]:subSet[[numVars]]),
#     obs.names=c(subSet[[1]]:subSet[[numVars]])) # parameter numbers or indication that parameter is fixed
#   
...
#     values.observed=diag(rep(0.1, 2)), #measurement noise in signals
#     params.observed=diag(c("mn1", "mn2"), 2))
#   
#   fcoupled=list(
#     list(var1 ~ p_libiself*var1 + p_rlibi*var2,
#          var2 ~  p_ribiself*var2 + p_lribi*var1))
#   
...
names(df) <- gsub("_",".", names(df), fixed=TRUE) #for consistency with below
...
cor(df[,c("l.dom.interp.detrend", "r.dom.interp.detrend", "l.aff.interp.detrend", "r.aff.interp.detrend")])
...
observed <- df[,c("l.dom.interp.detrend", "r.dom.interp.detrend", "l.aff.interp.detrend", "r.aff.interp.detrend")]
