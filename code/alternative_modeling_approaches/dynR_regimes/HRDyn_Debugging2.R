...
#basedir <- "/Users/admin_quc16/Box Sync/PD_Interaction_Dynamics/data"
...
tofit$time <- tofit$time-.25
#BEGIN DYNR MODELING USING TOFIT


coef_dynr <- lapply(, function(tofit) {
...
initial <- prep.initial(
  ...
  values.inicov = diag(10, 2), #starting value of 1 for signal covariance
...
aic <- AIC(res_sonly)
bic <- BIC(res_sonly)
return(sum, coef, loglik, aic, bic)
