function [ACQsampleRate,nVarSampleDivider,chanData] = readVarSampRateACQ(filename)
...
%
% ACQsampleRate: acquisition sampling rate
% nVarSampleDivider: nchan vector of sampling rate dividers (all ones for uniform sampling at ACQsampleRate)
% chanData: nchan x nsamp array of data
% for channel i only every nVarSampleDivider(i) sample is valid (others are
% set to zero
% filename: name of acquisition file without the .acq extension
% 
...
%
% 11-Nov-2005 generalized reading variable sampling rates (should work for any .acq file)
%
% 19-Jan-2006 changed name to readVarSampRateACQ; now also returns nVarSampleDivider
%
% 23-Sep-2006 corrected sectionstart
%
% 25 Nov 2008 corrected line 257 to handle version 3.7.0 which is saved from
% Mac version 3.9.1 for windows
%

%load local_path;
%filename = 'TRECALL_102_BASE_0005.acq';
%filename = 'Rox';
%filename = 'Bryan_setb_block1_10-21-05';
%get rid of .acq if still in filename
if strcmp(upper(filename(end-2:end)),'ACQ')
...
if header.lVersion<30 |  header.lVersion>50  %%%%%%%%%%%%%%%%%here
...
if length(divSamp) == 1  % all channels have same sample rate use block read
    ...
    chanData = zeros(nChannels,lBufLength);
    blockCategories = unique(blocksize);
    % set up cell array of channel numbers for each block category
    categoryChans = cell(1,length(blockCategories));
    for i=1:length(categoryChans)
        categoryChans{i} = find(nVarSampleDivider <= divSamp(i));
    end
    % read all channels for each sample block
    for iSamp = 1:lBufLength
        categoryType = contains(blockCategories,blocksize(iSamp));
            for ichan=categoryChans{categoryType}
                switch nType(ichan)
                    case 1  % double
