...
setwd("/Users/ams939/Box Sync/DEPENd/Projects/PD_Interaction_Dynamics/code")
...
ibifiles <- list.files(path="/Users/ams939/Desktop/vanBse_ibis", pattern=".*vanilla.*\\.txt", full.names=TRUE)

ids <- as.numeric(sub("/Users/ams939/Desktop/vanBse_ibis/(8[0-9]{3}).*", "\\1", ibifiles, perl=TRUE))


position <- sub("/Users/ams939/Desktop/vanBse_ibis/8[0-9]{3}_([lr])_.*", "\\1", ibifiles, perl=TRUE)
...
g <- ggplot(physio_split$`8023`, aes(x=time, y=ibi, fill = position)) + geom_line() + ggtitle(physio_split$`8023`$PTNUM[1])
plot(g)
...
#interpolateIBI is in interpolation_10feb
align_split_10Hz <- lapply(physio_split, function(id) {
      interpolateIBI(id)
    })
names(align_split_10Hz) <- names(physio_split)

#4Hz version
align_split_4Hz <- lapply(physio_split, function(id) {
  interpolateIBI(id, 4)
    })
names(align_split_4Hz) <- names(physio_split)

#2Hz version
align_split_2Hz <- lapply(physio_split, function(id) {
  interpolateIBI(id, 2)
    })
names(align_split_2Hz) <- names(physio_split)

#1Hz version
align_split_1Hz <- lapply(physio_split, function(id) {
  interpolateIBI(id, 1)
})
names(align_split_1Hz) <- names(physio_split)

align_split_1.5Hz <- lapply(physio_split, function(id) {
  interpolateIBI(id, 1.5)
    })
names(align_split_1.5Hz) <- names(physio_split)

save(align_split_10Hz, align_split_4Hz, align_split_2Hz, align_split_1Hz, align_split_1.5Hz, file="data/vanBse_align_split_atdifferentfreqs.RData")
...
save(alldata, file="align_split_atdifferentfreqs_patpar_vanBse.RData")
length(alldata)
align_split_patientpartner_10Hz <- alldata[[5]]
align_split_patientpartner_4Hz <- alldata[[4]]
#10Hz version

basedir <- "/Users/ams939/Box Sync/DEPENd/Projects/PD_Interaction_Dynamics/data/Nov_4Hz_data"

load("/Users/ams939/Box Sync/DEPENd/Projects/PD_Interaction_Dynamics/data/align_split_atdifferentfreqs_patpar_March2017.RData")
align_split_patientpartner_4Hz <- alldata[[4]]
lapply(align_split_patientpartner_4Hz, function(couple) {
  couple$time <- couple$ms / 250
