...
dfpersonalityDat <- dplyr::select(datClean2, one_of(c("iip_elcpr", "pdcountcpr", "pdcountcpt", "iip_x_pdcount_partner", "iip_x_pdcount_patient", "iip_elcpt", "PTNUM_patient", "PTNUM_partner")))
write.csv(dfpersonalityDat, file = "Subset of Data.csv", row.names = TRUE)
...
ls <- sapply(align_split_4Hz, function(x) {
  x$PTNUM[[1]]
})
ls <- c(ls,ls)
theta <- data.frame(ls, thetas3, thetas4)

#example of APIM --> need to change for actual data set
#runAPIM(personalityDat, "iip_bpd", "bordl_sidp", partners=c("_partner", "_patient"), printall=TRUE)

pDat <- filter(personalityDat, PTNUM %in% ls)
#pDatPTNUM <- distinct(pDat, PTNUM)
...
# pDat <- filter(personalityDat, PTNUM %in% ls)
# pDatPTNUM <- distinct(pDat, PTNUM)
# thetaFinal <- filter(theta, PTNUM %in% pDatPTNUM$PTNUM)
# pDat <- arrange(pDat, DyadID)
# pDat$self_coupling <- thetaFinal$thetas3
# pDat$cross_coupling <- thetaFinal$thetas4
# pDatPatient <- subset(pDat, DyadID == 1 )
# pDatPartner <- subset(pDat, DyadID == 0)
# pDatPatient <- rename(pDatPatient, self_coupling_patient = self_coupling)
# pDatPartner <- rename(pDatPartner, self_coupling_partner = self_coupling)
# pDatPatient <- rename(pDatPatient, allpdCount_patient = allpdCount)
# pDatPartner <- rename(pDatPartner, allpdCount_partner = allpdCount)
# #find a way to rename all columns in pDatPatient/Partner with _patient/_partner 
# pDatNew <- data.frame(pDatPatient, pDatPartner)


#now use APIM_functions to test different correlates of interest (i.e. self/cross_coupling as DV, change the IVs)

lattice::splom(dplyr::select(pDatNew, self_coupling_partner, self_coupling_patient, cross_coupling_partner, cross_coupling_patient))
library(ppcor)
lattice::splom(dplyr::select(datClean2, self_coupling_partner, self_coupling_patient, cross_coupling_partner, cross_coupling_patient))
...
runAPIM(pDatClean, "cross_coupling", "DASTotal", partners = c("_partner", "_patient"), printall = TRUE)
...
large_self_coupling_patient ~  iip_elevation_patient + p1*iip_elevation_partner + iip_x_pdcount_patient + pdcountcpt + pdcountcpr
large_self_coupling_partner ~ iip_elevation_partner + p1*iip_elevation_patient+ iip_x_pdcount_partner+ iip_x_pdcount_patient + pdcountcpt + pdcountcpr
large_cross_coupling_patient ~   iip_elevation_patient + p2*iip_elevation_partner + iip_x_pdcount_patient+ iip_x_pdcount_patient + pdcountcpt + pdcountcpr
large_cross_coupling_partner ~   iip_elevation_patient + p2*iip_elevation_partner + iip_x_pdcount_partner+iip_x_pdcount_patient + pdcountcpt + pdcountcpr
...
anova(iip_pdcount_moderation_apathsfreed, iip_elevation_pfreed)


iip_pdcount_moderationd <- "

large_self_coupling_patient ~  iip_elevation_patient  + iip_xpdcount_patient + allpdCount_patient 
large_self_coupling_partner ~ iip_elevation_partner +  iip_x_pdcount_partner+ allpdCount_partner 
large_cross_coupling_patient ~   iip_elevation_patient +  iip_xpdcount_patient+ allpdCount_patient 
large_cross_coupling_partner ~   iip_elevation_partner +  iip_x_pdcount_partner+ allpdCount_partner 

"

iip_pdcount_mod <- sem(iip_pdcount_moderationd, datClean2, missing = "listwise", estimator = "MLR", fixed.x = TRUE)
...
cor(datClean2[,c("allpdCount_patient", "allpdCount_partner", "iip_x_pdcount_patient", "iip_x_pdcount_partner", "iip_elevation_patient", "iip_elevation_part")])
